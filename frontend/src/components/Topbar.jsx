...
export function Topbar({ onMenu }) {
  ...
  const pageTitle = useMemo(() => {
    ...
    if (p.startsWith('/project')) return 'Project Dashboard'
    if (p.startsWith('/team')) return 'Project Team'
    if (p.startsWith('/workflow')) return 'Workflow'
    if (p.startsWith('/field')) return 'Field Survey & Mobile'
    if (p.startsWith('/collaboration')) return 'Collaboration Portal'
    if (p.startsWith('/workspace')) return 'Map Workspace'
    if (p.startsWith('/subdivision')) return 'AI Land Subdivision'
